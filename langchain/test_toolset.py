"""Tests for latchgate-langchain.

Unit tests are self-contained — no running LatchGate instance required.
HTTP discovery is mocked via httpx.MockTransport injected through the
``_http`` parameter, and the LatchGateClient is mocked at the SDK level.
"""
...
class TestSchemaConversion:
    ...
    def test_required_field_has_no_default(self) -> None:
        model = schema_to_pydantic("http_fetch", SAMPLE_HTTP_FETCH_SCHEMA)
        url_field = model.model_fields["url"]
        assert url_field.is_required()

    def test_optional_field_has_default(self) -> None:
        model = schema_to_pydantic("http_fetch", SAMPLE_HTTP_FETCH_SCHEMA)
        method_field = model.model_fields["method"]
        assert not method_field.is_required()
        assert method_field.default == "GET"

    def test_optional_field_without_explicit_default_is_none(self) -> None:
        model = schema_to_pydantic("http_fetch", SAMPLE_HTTP_FETCH_SCHEMA)
        headers_field = model.model_fields["headers"]
        assert not headers_field.is_required()
        assert headers_field.default is None

    def test_descriptions_preserved(self) -> None:
        model = schema_to_pydantic("http_fetch", SAMPLE_HTTP_FETCH_SCHEMA)
        url_field = model.model_fields["url"]
        assert url_field.description == "Target URL"
    ...
    def test_permissive_fallback_schema(self) -> None:
        model = schema_to_pydantic("fallback", {"type": "object", "additionalProperties": True})
        assert issubclass(model, BaseModel)
    ...
    def test_union_type_takes_first_non_null(self) -> None:
        ...
        model = schema_to_pydantic("union_test", schema)
        # Should resolve to str (first non-null)
    ...
    def test_unknown_type_defaults_to_str(self) -> None:
        ...
        model = schema_to_pydantic("unknown_type", schema)
        instance = model(x="test")
        assert instance.x == "test"

    def test_generated_model_produces_valid_json_schema(self) -> None:
        """The generated Pydantic model should produce a valid JSON schema
        that LangChain can use for function-call descriptions."""
        model = schema_to_pydantic("http_fetch", SAMPLE_HTTP_FETCH_SCHEMA)
...
class TestDescriptionBuilder:
    ...
    def test_default_redacts_database_details(self) -> None:
        ...
        assert "update_order_status" not in desc
    ...
    def test_debug_includes_database(self) -> None:
        ...
        assert "update_order_status" in desc
...
class TestToolExecution:
    ...
    @pytest.mark.asyncio
    async def test_denied_raises_tool_exception(self) -> None:
        client = AsyncMock(spec=LatchGateClient)
        client.execute = AsyncMock(side_effect=LatchGateDenied("http_fetch", "policy_violation"))
        tool = _make_tool(client)

        result = await tool.ainvoke({"url": "https://evil.com"})
        assert "denied" in result.lower()
        assert "policy_violation" in result

    @pytest.mark.asyncio
    async def test_approval_required_raises_tool_exception(self) -> None:
        client = AsyncMock(spec=LatchGateClient)
    ...
    @pytest.mark.asyncio
    async def test_budget_exhausted_raises_tool_exception(self) -> None:
        client = AsyncMock(spec=LatchGateClient)
    ...
    @pytest.mark.asyncio
    async def test_unavailable_raises_tool_exception(self) -> None:
        client = AsyncMock(spec=LatchGateClient)
    ...
    def test_handle_tool_error_is_true_by_default(self) -> None:
        client = AsyncMock(spec=LatchGateClient)
        tool = _make_tool(client)
    ...
    async def test_execution_passes_all_kwargs(self) -> None:
        ...
        await tool.ainvoke({"url": "https://example.com", "method": "HEAD", "timeout_seconds": 5})
    ...
    async def test_audit_callback_not_invoked_on_error(self) -> None:
        ...
        await tool.ainvoke({"url": "https://evil.com"})
...
class TestDiscovery:
    ...
    async def test_discover_all_actions(self) -> None:
        ...
        assert ids == {"http_fetch", "send_message", "database"}
    ...
    @pytest.mark.asyncio
    async def test_detail_fetch_failure_still_produces_descriptor(self) -> None:
        http = _mock_http(
            {
                "/v1/actions": {
                    "actions": [
                        {"action_id": "test_action", "version": "1.0.0", "risk_level": "low"}
                    ]
                },
                "/v1/actions/test_action/schema/request": {
                    "type": "object",
        ...
        assert len(descriptors) == 1
        assert descriptors[0].action_id == "test_action"
        assert descriptors[0].declared_side_effects == []
        assert "audit receipts" in descriptors[0].description

    @pytest.mark.asyncio
    async def test_trailing_slash_in_gate_url_is_normalized(self) -> None:
        http = _mock_http(
            {
                "/v1/actions": {
    ...
    @pytest.mark.asyncio
    async def test_descriptor_fields_populated_correctly(self) -> None:
        http = _mock_http(
            {
                "/v1/actions": {
                    "actions": [
        ...
        assert d.action_id == "http_fetch"
        assert d.version == "1.0.0"
        assert d.risk_level == "low"
        assert d.declared_side_effects == ["http_read"]
        assert "url" in d.request_schema.get("properties", {})
...
class TestToolset:
    ...
    async def test_create_with_mocked_discovery(self) -> None:
        """Test the full Toolkit.create() flow with mocked HTTP."""
        http = _mock_http(_full_discovery_routes())
    ...
    @pytest.mark.asyncio
    async def test_from_descriptors(self) -> None:
        client = AsyncMock(spec=LatchGateClient)
        descriptors = [
            ActionDescriptor(
                action_id="http_fetch",
    ...
    @pytest.mark.asyncio
    async def test_toolset_multiple_tools(self) -> None:
        client = AsyncMock(spec=LatchGateClient)
        descriptors = [
            ActionDescriptor(
        ...
        toolset = LatchGateToolset.from_descriptors(descriptors, client=client)
        assert len(toolset.get_tools()) == 5
        assert len(toolset.action_ids) == 5

    @pytest.mark.asyncio
    async def test_toolset_falls_back_to_default_transport(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_toolset_discovers_via_client_transport(self) -> None:
        ...
        assert len(toolset.get_tools()) == 3

    @pytest.mark.asyncio
    async def test_toolset_context_manager(self) -> None:
        client = AsyncMock(spec=LatchGateClient)
        descriptors = [
            ActionDescriptor(
                action_id="test",
        ...
        async with LatchGateToolset.from_descriptors(descriptors, client=client) as toolset:
            assert len(toolset.get_tools()) == 1

        client.close.assert_awaited_once()

    @pytest.mark.asyncio
    async def test_toolset_exposes_client(self) -> None:
        client = AsyncMock(spec=LatchGateClient)
    ...
    @pytest.mark.asyncio
    async def test_toolset_empty_descriptors(self) -> None:
        client = AsyncMock(spec=LatchGateClient)
        toolset = LatchGateToolset.from_descriptors([], client=client)
        assert toolset.get_tools() == []
        assert toolset.action_ids == []

    @pytest.mark.asyncio
    async def test_toolset_get_tools_returns_copy(self) -> None:
        """get_tools() should return a new list each time, not a mutable reference."""
        client = AsyncMock(spec=LatchGateClient)
        descriptors = [
            ActionDescriptor(
                action_id="test",
        ...
        toolset = LatchGateToolset.from_descriptors(descriptors, client=client)
        tools_a = toolset.get_tools()
        tools_b = toolset.get_tools()
        assert tools_a is not tools_b
        assert tools_a == tools_b

    @pytest.mark.asyncio
    async def test_toolset_with_include_filter(self) -> None:
        """Toolkit.create() respects include filter — only specified actions become tools."""
        http = _mock_http(
            {
                "/v1/actions": SAMPLE_ACTIONS_RESPONSE,
                "/v1/actions/http_fetch/schema/request": SAMPLE_HTTP_FETCH_SCHEMA,
